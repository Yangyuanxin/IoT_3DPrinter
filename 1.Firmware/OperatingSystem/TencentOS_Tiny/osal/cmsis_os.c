...
osThreadId osThreadCreate(const osThreadDef_t *thread_def, void *argument)
{
	...
	{
		k_task_t *task;
		err = tos_task_create_dyn(&task, thread_def->name,
				(k_task_entry_t) thread_def->pthread, argument,
				priority_cmsis2knl(thread_def->tpriority),
				thread_def->stacksize, thread_def->timeslice);
		return err == K_ERR_NONE ? task : NULL;
	}
#endif

	err = tos_task_create((k_task_t *) thread_def->task, thread_def->name,
			(k_task_entry_t) thread_def->pthread, argument,
			priority_cmsis2knl(thread_def->tpriority), thread_def->stackbase,
			thread_def->stacksize, thread_def->timeslice);
...
{
	return errno_knl2cmsis(
			tos_task_prio_change((k_task_t *) thread_id,
					priority_cmsis2knl(priority)));
}
...
osStatus osMutexWait(osMutexId mutex_id, uint32_t millisec)
{
	...
	return errno_knl2cmsis(
			tos_mutex_pend_timed((k_mutex_t *) mutex_id, timeout));
}
		...
		osThreadId thread_id)
{
	...
	err = tos_msg_q_create((k_msg_q_t *) queue_def->queue, queue_def->pool,
			queue_def->queue_sz);
