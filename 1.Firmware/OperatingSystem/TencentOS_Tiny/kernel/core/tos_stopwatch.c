...
__API__ k_err_t tos_stopwatch_countdown_ms(k_stopwatch_t *stopwatch,
		k_time_t millisec)
{
	k_tick_t tick;
...
__API__ k_time_t tos_stopwatch_remain_ms(k_stopwatch_t *stopwatch)
{
	...
	now = tos_systick_get();
	return (k_time_t) (((stopwatch->until) - now + TOS_CFG_CPU_TICK_PER_SECOND
			- 1) / TOS_CFG_CPU_TICK_PER_SECOND);
}
