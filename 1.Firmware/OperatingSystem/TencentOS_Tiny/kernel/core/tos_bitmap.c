...
__API__ k_err_t tos_bitmap_create_empty(k_bitmap_t *bitmap,
		k_bmtbl_t *bitmap_tbl, uint32_t bit_max)
{
	int i = 0;
...
__API__ k_err_t tos_bitmap_create_full(k_bitmap_t *bitmap,
		k_bmtbl_t *bitmap_tbl, uint32_t bit_max)
{
	int i = 0;
...
__API__ int tos_bitmap_is_set(k_bitmap_t *bitmap, uint32_t bit)
{
	...
	return (bitmap->bitmap_tbl[K_BITMAP_NDX(bit)] & K_BITMAP_BIT(bit)) ?
			K_TRUE : K_FALSE;
}
