...
static int sockaddr2ip(const struct sockaddr_in *addr_in, char ip[], int ip_len)
{
	snprintf(ip, 16, "%u.%u.%u.%u",
			((uint8_t *) (&addr_in->sin_addr.s_addr))[0],
...
static int sockaddr2port(const struct sockaddr_in *addr_in, char port[],
		int port_len)
{
...
int recv(int socket, void *buffer, size_t length, int flags)
{
	...
	{
		return tos_sal_module_recv_timeout(socket_get_fd(socket), buffer,
				length, 4000);
	}
...
int sendto(int socket, const void *message, size_t length, int flags,
